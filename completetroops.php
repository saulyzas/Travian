...
while($row = mysql_fetch_array($sql)){
$gold = $row["gold"];};
if ($gold < 100 ) {die('<a href="http://travian.us.lt/plus1.php?id=111">Not enough gold. Buy gold</a>');}
else {
...
mysql_query("UPDATE  `".TB_PREFIX."units` SET `u".$unit_w['unit']."`=`u".$unit_w['unit']."`+'".$unit_w['amt']."'  where `vref`='".$_SESSION['wid']."'  ");
 ...
 mysql_query("UPDATE ".TB_PREFIX."users set gold = ".($session->gold-100)." where `id`='".$session->uid."'") or die(mysql_error());
